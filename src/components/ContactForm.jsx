  ...
  return (
    <div className="flex flex-col md:flex-row bg-gradient-to-r from-blue-200 via-blue-100 to-blue-200 gap-6 items-center py-8">
      ...
      <div className="w-full md:w-1/2">
        ...
        <form className="flex flex-col gap-6 w-[80%] mx-auto pt-4" onSubmit={handleSubmit}>
          <div className="flex flex-col md:flex-row gap-6">
            <Input type="text" label="Your Full Name" value={fullName} onChange={(e) => setFullName(e.target.value)}/>
            <Input type="email" label="Your Email" value={email} onChange={(e) => setEmail(e.target.value)}/>
          </div>
          <div className="w-full">
            <Input type="text" label="Subject" value={subject} onChange={(e) => setSubject(e.target.value)}/>
          </div>
          <div className="w-full">
            <Textarea label="Message" value={message} onChange={(e) => setMessage(e.target.value)}/>
          </div>
          <button type="submit" className="w-48 bg-blue-900 py-2 rounded-sm text-blue-50 tracking-wider font-semibold">
            Pošaljite
          </button>
        </form>
      </div>
